...
pb:
	protoc --go_out=. --go-grpc_out=. --go_opt=paths=source_relative --go-grpc_opt=paths=source_relative api/pb/rating.proto

dev:
	docker-compose -f docker-compose.yml up

dev-db:
	docker run --rm --name postgres_ratings_db \
	-p 5432:5432 \
	-e POSTGRES_PASSWORD=794613825Zx! \
	-e POSTGRES_DB=ratings_db \
